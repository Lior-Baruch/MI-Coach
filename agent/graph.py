...
and the judge (questionnaire registry + structured scoring) in ``agent.judging``.
"""
...
def judge_turn_node(state: SessionState) -> dict:
    ...
    selected = filter_known(state.get("turn_questionnaires") or DEFAULT_TURN_QUESTIONNAIRES)
    usage = dict(state.get("openai_usage") or empty_usage())
    rationale = bool(state.get("turn_rationale"))
    results = {name: judge_named(name, conv, state.get("params"), rationale, usage)
...
def report_node(state: SessionState) -> dict:
    ...
    selected = filter_known(state.get("report_questionnaires") or DEFAULT_REPORT_QUESTIONNAIRES)
    results = {name: judge_named(name, conv, p, rationale, usage) for name in selected}
